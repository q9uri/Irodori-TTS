...
def upcycle_interleaved_24_layers(input_path, output_path):
    ...
    for key, value in state_dict.items():
        if key.startswith("blocks."):
            ...
            key_a_split = [split_key[0], str(index *3)] + split_key[2:]
            key_b_split = [split_key[0], str(index *3 + 1)] + split_key[2:]
            key_c_split = [split_key[0], str(index *3 + 2)] + split_key[2:]

            
            new_key_a = ".".join(key_a_split)
            new_key_b = ".".join(key_b_split)
            new_key_c = ".".join(key_c_split)
                
            new_state_dict[new_key_a] = value.clone()
            new_state_dict[new_key_b] = value.clone()
            new_state_dict[new_key_c] = value.clone()
        else:
...
# 実行例
upcycle_interleaved_24_layers("./models/Irodori-TTS-500M-v2.safetensors", "./models/Irodori-TTS-36L-interleaved.safetensors")
